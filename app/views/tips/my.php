...
<div class="w-full">
    <div class="mx-auto w-full max-w-4xl rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div class="mb-6 flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-slate-900">Mẹo vặt của tôi</h1>
                <p class="text-sm text-slate-500">Theo dõi trạng thái duyệt và gửi lại nếu bị từ chối.</p>
            </div>
            <a class="rounded-xl bg-primary px-4 py-2 text-sm font-semibold text-white" href="<?= URLROOT; ?>/tips/create">Góp ý mới</a>
        </div>

        <?php if (empty($tips)): ?>
            <div class="rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-500">Bạn chưa gửi mẹo vặt nào.</div>
        <?php else: ?>
            <div class="divide-y divide-slate-100">
                <?php foreach ($tips as $tip): ?>
                    <?php $status = (string) ($tip['status'] ?? 'pending'); ?>
                    <div class="py-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                        ...
                        <div class="flex items-center gap-2">
                            <a class="rounded-md border border-slate-200 px-3 py-1 text-xs font-semibold text-slate-600" href="<?= URLROOT; ?>/tips/<?= htmlspecialchars((string) ($tip['slug'] ?? ''), ENT_QUOTES, 'UTF-8'); ?>">Xem</a>
                            <?php if ($status === 'rejected'): ?>
                                <form method="post" action="<?= URLROOT; ?>/tips/<?= (int) $tip['id']; ?>/resubmit">
                                    <?= csrf_field(); ?>
