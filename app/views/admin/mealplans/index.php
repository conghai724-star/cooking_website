...
$noticeText = match ($notice) {
    'deleted' => 'Da xoa muc thuc don vi pham.',
    'delete_failed' => 'Không thể xoa muc thuc don.',
    default => '',
};
...
<div class="flex flex-col gap-6">
    <div class="flex flex-col gap-2">
        ...
        <p class="text-sm text-slate-500">Xem va xu ly ke hoach thuc don cua người dùng.</p>
    </div>
    ...
    <div class="rounded-lg border border-slate-200 bg-white p-4">
        <form method="get" action="<?= URLROOT; ?>/admin/mealplans" class="flex flex-wrap items-end gap-3">
            <label class="flex flex-col gap-1 text-xs font-semibold text-slate-600">
                Tu khoa
                <input type="text" name="q" value="<?= $e($keyword); ?>" class="w-72 rounded-lg border border-slate-300 px-3 py-2 text-sm font-normal" placeholder="Ten user, email, ten công thức">
            </label>
            <label class="flex flex-col gap-1 text-xs font-semibold text-slate-600">
                User ID
    ...
    <div class="rounded-lg border border-slate-200 bg-white overflow-hidden">
        <div class="flex items-center justify-between border-b border-slate-100 px-4 py-3">
            <h3 class="font-semibold text-slate-800">Danh sach thuc don (<?= $total; ?>)</h3>
        </div>
            ...
            <table class="w-full text-left text-sm">
                <thead class="bg-slate-50 text-slate-600">
                    <tr>
                        ...
                        <th class="px-4 py-2 font-semibold">Nguoi dung</th>
                        <th class="px-4 py-2 font-semibold">Ngay</th>
                        <th class="px-4 py-2 font-semibold">Buoi</th>
                        <th class="px-4 py-2 font-semibold">Vai tro mon</th>
                        <th class="px-4 py-2 font-semibold">Cong thuc</th>
                        <th class="px-4 py-2 font-semibold">Tao luc</th>
                        <th class="px-4 py-2 font-semibold">Xu ly</th>
                    </tr>
                </thead>
                            ...
                            <tr>
                                ...
                                <td class="px-4 py-2">
                                    <?php if ($canModerateMealPlans): ?>
                                        <form method="post" action="<?= URLROOT; ?>/admin/mealplans/<?= (int) ($row['id'] ?? 0); ?>/delete" onsubmit="return confirm('Xoa muc thuc don nay?');">
                                            <?= csrf_field(); ?>
                                            <button class="rounded border border-rose-300 px-2 py-1 text-xs font-semibold text-rose-700 hover:bg-rose-50" type="submit">Xoa vi pham</button>
                                        </form>
                                    <?php else: ?>
            ...
            <div class="flex items-center justify-between border-t border-slate-100 px-4 py-3 text-sm">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <?php if ($page > 1): ?>
                        ...
                        <a class="rounded border border-slate-300 px-3 py-1 text-slate-700 hover:bg-slate-50" href="<?= URLROOT; ?>/admin/mealplans?<?= http_build_query($baseParams); ?>">Prev</a>
                    <?php endif; ?>
                    <?php if ($page < $totalPages): ?>
                        <?php $baseParams['page'] = $page + 1; ?>
                        <a class="rounded border border-slate-300 px-3 py-1 text-slate-700 hover:bg-slate-50" href="<?= URLROOT; ?>/admin/mealplans?<?= http_build_query($baseParams); ?>">Next</a>
                    <?php endif; ?>
                </div>
            </div>
        <?php endif; ?>
