...
<section class="w-full">
    <div class="mx-auto w-full max-w-4xl rounded-2xl border border-slate-200 bg-white p-5 sm:p-6">
        <h1 class="text-2xl font-bold text-slate-900">Sửa hồ sơ</h1>
        <p class="mt-1 text-sm text-slate-500">Cập nhật thông tin cá nhân của bạn.</p>
        ...
        <form class="mt-5 space-y-5" method="post" action="<?= URLROOT; ?>/profile/edit" enctype="multipart/form-data">
            ...
            <section class="space-y-4" data-tab-panel="info">
                <div>
                    <label class="mb-1 block text-sm font-semibold text-slate-700" for="name">Tên</label>
                ...
                <div>
                    <label class="mb-1 block text-sm font-semibold text-slate-700" for="bio">Giới thiệu</label>
                ...
                <div>
                    <p class="mb-2 block text-sm font-semibold text-slate-700">Ảnh đại diện</p>
                    <div class="flex items-center gap-3">
                        <?php if ($avatarUrl !== ''): ?>
                            <img class="h-14 w-14 rounded-full object-cover" src="<?= htmlspecialchars($avatarUrl, ENT_QUOTES, 'UTF-8'); ?>" alt="Ảnh đại diện hiện tại">
                        <?php else: ?>
            ...
            <section class="space-y-4 hidden" data-tab-panel="password">
                <div>
                    <label class="mb-1 block text-sm font-semibold text-slate-700" for="current_password">Mật khẩu hiện tại (bắt buộc khi đổi email hoặc đổi mật khẩu)</label>
                ...
                <div>
                    <label class="mb-1 block text-sm font-semibold text-slate-700" for="new_password">Mật khẩu mới</label>
                ...
                <div>
                    <label class="mb-1 block text-sm font-semibold text-slate-700" for="confirm_new_password">Xác nhận mật khẩu mới</label>
            ...
            <section class="space-y-4 hidden" data-tab-panel="email">
                <div>
                    ...
                    <p class="mt-1 text-xs text-slate-400">Nhập email mới, sau đó xác nhận bằng mật khẩu hiện tại bên dưới.</p>
                </div>

                <div>
                    <label class="mb-1 block text-sm font-semibold text-slate-700" for="email_current_password">Mật khẩu hiện tại (xác nhận đổi email)</label>
            ...
            <div class="flex items-center gap-3 pt-2">
                <button class="rounded-lg bg-primary px-4 py-2 text-sm font-bold text-white hover:bg-amber-500" type="submit">Lưu thay đổi</button>
                <a class="text-sm font-semibold text-slate-500 hover:text-slate-900" href="<?= URLROOT; ?>/profile">Hủy</a>
            </div>
        </form>
    </div>
</section>
